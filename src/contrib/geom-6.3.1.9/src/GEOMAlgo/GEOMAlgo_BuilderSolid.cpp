  ...
  void GEOMAlgo_BuilderSolid::PerformInternalShapes()
{
  ...
  for ( ; aSolidIt.More(); aSolidIt.Next()) {
    ...
    for (; aItMF.More(); aItMF.Next()) {
      ...
      if (GEOMAlgo_Tools3D::IsInternalFace(aF, aSolid, aMEF, 1.e-14, (IntTools_Context&)myContext)) {
	aMFP.Add(aF);
      }
    }
    //
...
Standard_Boolean IsHole(const TopoDS_Shape& theS2,
			const Handle_IntTools_Context& theContext)
{
			  ...
			  const Handle_IntTools_Context& theContext)
{
  ...
  if (!aExp.More()){
    ...
    aState=GEOMAlgo_Tools3D::ComputeState(aF, *pS2, 1.e-14, aBounds, (IntTools_Context&)theContext);
  }
