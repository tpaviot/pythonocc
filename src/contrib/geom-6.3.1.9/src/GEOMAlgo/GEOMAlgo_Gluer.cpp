      ...
      if (!myOrigins.IsBound(theShape))   {
        ...
        for (; aExpE.More(); aExpE.Next()) {
          ...
          if (!BRep_Tool::Degenerated(aER)) {
            ...
            //
            // orient image
            Standard_Boolean bIsToReverse=BOPTools_Tools3D::IsSplitToReverse1(aER, aE, (Handle_IntTools_Context&)myContext);
            if (bIsToReverse) {
              aER.Reverse();
            }
          }
			      ...
			      TopoDS_Face& aNewFace)
{
  ...
  for (; aExpW.More(); aExpW.Next()) {
	...
	// orient image 
	bIsToReverse=BOPTools_Tools3D::IsSplitToReverse1(aER, aE, (Handle_IntTools_Context&)myContext);
	if (bIsToReverse) {
	  aER.Reverse();
	}
      }
