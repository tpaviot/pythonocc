...
#endif
#ifndef _SMESH_subMesh_HeaderFile
...
class SMESH_MeshVSLink : public MeshVS_DataSource3D {
  ...
  public:
	...
	//! Constructor <br>
  Standard_EXPORT SMESH_MeshVSLink(const SMESH_Mesh *aMesh, SMESH_subMesh *onlySubMeshElems=NULL); // M.B. added option onlySubMeshElems
  Standard_EXPORT void GetHiddenNodes(int planedir, double value, TColStd_PackedMapOfInteger& hiddennodes);
  Standard_EXPORT void GetHiddenElemes(TColStd_PackedMapOfInteger& hiddennodes, TColStd_PackedMapOfInteger& hiddenelems, bool VolElems); 
  ...
  private:
	...
	// Fields PRIVATE
  //
  // std::map<Standard_Integer, std::vector<Standard_Integer> > Node2ElemMap;
  std::vector<XYZ_> ElemCentroids;
  std::vector<Standard_Integer> ElemIDs;
  int _planedir;
  double _value;
	SMESH_Mesh *myMesh;
