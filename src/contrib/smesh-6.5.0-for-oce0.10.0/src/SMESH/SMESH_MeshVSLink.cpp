...
// Purpose  :
//================================================================
SMESH_MeshVSLink::SMESH_MeshVSLink(const SMESH_Mesh *aMesh, SMESH_subMesh *onlySubMeshElems)
{
  SMESHDS_SubMesh* submeshds = NULL; 

  // M.B. to allow for independant visualization of submeshes with the OCC viewer
  if (onlySubMeshElems!=NULL)
    submeshds = onlySubMeshElems->CreateSubMeshDS();
  ...
  for(;anEdgeIter->more();) {
	...
	else myElements.Add( anElem->GetID() );
  }
  //add the faces
	...
	if (submeshds!=NULL) // M.B.
	  {
		...
		double X=0.; double Y=0.; double Z=0.;  int count=0;
		while ( nodeIt->more() ) {
		  const SMDS_MeshNode* node = static_cast<const SMDS_MeshNode*>( nodeIt->next() );
		  if (node)
		    {
		      X += node->X(); Y += node->Y(); Z += node->Z(); count++;
		    }
		}
		X /= count; Y /= count; Z /= count;
		ElemIDs.push_back(anElem->GetID());
		ElemCentroids.push_back(XYZ_(X,Y,Z));
	      }
	  }
	else myElements.Add( anElem->GetID() );
  }
...
void SMESH_MeshVSLink::GetHiddenNodes(int planedir, double value, TColStd_PackedMapOfInteger& hiddennodes)
{  
  _planedir=planedir;
  _value=value;
  ...
  for ( ; anIter.More(); anIter.Next() )
    {
      ...
      // get the node 
      const SMDS_MeshNode* myNode = myMesh->GetMeshDS()->FindNode(aKey);
      if (myNode)
	{
	  if (planedir==1)
	    {
...
void SMESH_MeshVSLink::GetHiddenElemes(TColStd_PackedMapOfInteger& hiddennodes, TColStd_PackedMapOfInteger& hiddenelems, bool VolElems) 
{
  for (int i=0; i<ElemIDs.size(); i++)
    {
      if (_planedir==1)
	{
	  if (ElemCentroids[i].x < _value) hiddenelems.Add(ElemIDs[i]);
	}
      else if (_planedir==2)
	{
	  if (ElemCentroids[i].y < _value) hiddenelems.Add(ElemIDs[i]);
	}
      else if (_planedir==3)
	{
	  if (ElemCentroids[i].z < _value) hiddenelems.Add(ElemIDs[i]);
	}
      else if (_planedir==4)
	{
	  if (ElemCentroids[i].x > _value) hiddenelems.Add(ElemIDs[i]);
