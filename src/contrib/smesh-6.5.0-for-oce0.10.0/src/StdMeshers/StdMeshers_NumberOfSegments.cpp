    ...
    throw (SMESH_Exception)
{
  ...
  {
    if( !syntax )
      throw SMESH_Exception(SMESH_Comment("invalid expression syntax: ") << str );
    if( !args )
      throw SMESH_Exception(LOCALIZED("only 't' may be used as function argument"));
    if( !non_neg )
      throw SMESH_Exception(LOCALIZED("only non-negative function can be used"));
    if( singulars )
    {
      char buf[1024];
    ...
    if( !non_zero )
      throw SMESH_Exception(LOCALIZED("f(t)=0 cannot be used"));
  }
