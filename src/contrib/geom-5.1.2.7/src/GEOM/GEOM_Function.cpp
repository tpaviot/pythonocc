...
void GEOM_Function::SetReal(int thePosition, const TCollection_AsciiString& theValue)
{
  ...
  if (!GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), theValue, theRealValue))
	Standard_Failure::Raise("One or more expressions cannot be avaluated");
								  ...
								  const Handle(TColStd_HArray1OfAsciiString)& theStringArray)
{
  ...
  {
	if (!GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), theStringArray->Value(itr), aRealValue))
	  Standard_Failure::Raise("One or more expressions cannot be evaluated");
...
double GEOM_Function::GetReal(int thePosition)
{
  ...
  if(anArgLabelExpr.FindAttribute(TDataStd_Expression::GetID() , anExpression  ))
  {
	...
	if (GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), theValueExpr, theRealValue))
	  TDataStd_Real::Set(anArgLabel, theRealValue);
  }
...
Handle(TColStd_HArray1OfReal) GEOM_Function::GetRealArray(int thePosition)
{
  ...
  if(anArgLabelExpr.FindAttribute(TDataStd_ExtStringArray::GetID() , aTDFStringArray  ))
  {
	...
	{
	  if (GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), aTDFStringArray->Value(itr), aRealValue))
...
void GEOM_Function::SetInteger(int thePosition, const TCollection_AsciiString& theValue)
{
  ...
  if (!GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), theValue, theDoubleToIntValue))
	Standard_Failure::Raise("One or more expressions cannot be avaluated");
									 ...
									 const Handle(TColStd_HArray1OfAsciiString)& theArray)
{
  ...
  {
	if (!GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), theArray->Value(itr), theDoubleToIntValue))
	  Standard_Failure::Raise("One or more expressions cannot be avaluated");
...
int GEOM_Function::GetInteger(int thePosition)
{
  ...
  if(anArgLabelExpr.FindAttribute(TDataStd_Expression::GetID() , anExpression  ))
  {
	...
	if (GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), theValueExpr, theDoubleToIntValue))
	  TDataStd_Integer::Set(anArgLabel, (Standard_Integer) theDoubleToIntValue);
  }
...
Handle(TColStd_HArray1OfInteger) GEOM_Function::GetIntegerArray(int thePosition)
{
  ...
  if(anArgLabelExpr.FindAttribute(TDataStd_ExtStringArray::GetID() , aTDFStringArray  ))
  {
	...
	for (int itr = aTDFStringArray->Lower(); itr <= aTDFStringArray->Upper(); itr ++)
	{
	  if (GEOM_Engine::GetEngine()->GetInterpreterEquationValue(GetDocumentID(), aTDFStringArray->Value(itr), theDoubleToIntValue))
