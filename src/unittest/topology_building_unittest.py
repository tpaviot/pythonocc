...
class TestTopologyBuilding(unittest.TestCase):
    ...
    def test_wire(self):
        ...
        Elips = gp_Elips(gp_Ax2(gp_Pnt(250., 0., 0.),
                                gp_Dir(1., 1., 1.)), 160., 90.)
        ...
        circle = gp_Circ(gp_Ax2(gp_Pnt(-300., 0., 0.),
                         gp_Dir(1., 0., 0.)), 80.)
        Edge2 = BRepBuilderAPI_MakeEdge(circle, 0, math.pi).Edge()
        ExistingWire = BRepBuilderAPI_MakeWire(Edge2).Wire()
        Edge3 = BRepBuilderAPI_MakeEdge(gp_Pnt(-300., 0., -80.),
                                        gp_Pnt(-90., 20., -30.)).Edge()
    ...
    def test_face(self):
        ...
        # The white Face
        sphere = gp_Sphere(gp_Ax3(gp_Pnt(0., 0., 0.),
                           gp_Dir(1., 0., 0.)), 150.)
        ...
        red_face = BRepBuilderAPI_MakeFace(curve, 0.001)
        ...
        BrownFace = BRepBuilderAPI_MakeFace(yellow_wire)
        ...
        bspline_surf = GeomAPI_PointsToBSplineSurface(array2, 3, 8,
                                                      GeomAbs_C2,
                                                      0.001).Surface()

        a_face = BRepBuilderAPI_MakeFace(bspline_surf, 0.001).Face()
        # 2d lines
        ...
        edge1 = BRepBuilderAPI_MakeEdge(line1.GetHandle(),
                                        bspline_surf, 0,
                                        p12d.Distance(p22d)).Edge()
        edge2 = BRepBuilderAPI_MakeEdge(line2.GetHandle(),
                                        bspline_surf, 0,
                                        p22d.Distance(p32d)).Edge()
        edge3 = BRepBuilderAPI_MakeEdge(line3.GetHandle(),
                                        bspline_surf, 0,
                                        p32d.Distance(p12d)).Edge()
        wire1 = BRepBuilderAPI_MakeWire(edge1, edge2, edge3).Wire()
        wire1.Reverse()
        pink_face = BRepBuilderAPI_MakeFace(a_face, wire1).Face()
    ...
    def test_evolved_shape(self):
        ...
        wprof = BRepBuilderAPI_MakePolygon(gp_Pnt(0., 0., 0.),
                                           gp_Pnt(-60., -60., -200.))
        S = BRepOffsetAPI_MakeEvolved(P.Wire(), wprof.Wire(),
                                      GeomAbs_Arc, True, False, True, 0.0001)
        S.Build()
