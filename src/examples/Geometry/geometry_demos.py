...
def make_text(string, pnt, height):
    ...
    # now we have to make a presenation for a stupid sphere as a workaround
    # to get to the object
#============================================================================
#     The reason for recreating is that myGroup is gone after an EraseAll call
#============================================================================
    basic_sphere = BRepPrimAPI_MakeSphere(1, 1, 1, 1)
    prs_sphere = AIS_Shape(basic_sphere.Shape())
...
def point_from_projections(event=None):
    ...
    if NbResults > 0:
        for i in range(1, NbResults+1):
            Q = PPS.Point(i)
            distance = PPS.Distance(i)
            pstring = "Q" + repr(i) + ": at Distance :" + PPS.Distance(i)
            make_text(pstring, Q, 6)
...
def bezier_surfaces(event=None):
    ...
    [BZ1, BZ2, BZ3, BZ4] = map(Geom_BezierSurface, [array1,
                                                    array2,
                                                    array3,
                                                    array4])
