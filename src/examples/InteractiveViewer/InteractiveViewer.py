...
def GetRecentFiles(fname):
    '''Open a text file which contains details of the
...
class AppFrame(wx.Frame):
    def __init__(self, parent):
        ...
        self._mgr.Update()
        # now make it so that the help pane can be resized
        self._mgr.GetPane("Help").MinSize((-1, -1))
        ...
        self.Bind(wx.EVT_MENU_RANGE, self.OnClickRecentFile, id=wx.ID_FILE1,
                  id2=wx.ID_FILE9)
        ...
        viewmenu.Append(restoreperspectiveID, u'Restore default layout',
                        'Restore the UI to the default layout.')
        self.Bind(wx.EVT_MENU, self.OnRestoreDefaultPerspective,
                  id=restoreperspectiveID)
        ...
        s_vertex = wx.NewId()
        selection_menu.Append(s_vertex, 'Vertex', 'Select vertices.')
        self.Bind(wx.EVT_MENU, self.OnSelectionVertex, id=s_vertex)
        s_edge = wx.NewId()
        selection_menu.Append(s_edge, 'Edge', 'Select edges.')
        self.Bind(wx.EVT_MENU, self.OnSelectionEdge, id=s_edge)
        s_face = wx.NewId()
        selection_menu.Append(s_face, 'Face', 'Select faces.')
        self.Bind(wx.EVT_MENU, self.OnSelectionFace, id=s_face)
        s_neutral = wx.NewId()
        selection_menu.Append(s_neutral, 'Neutral',
                              'Switch back to global shapes selction.')
        ...
        displaymode_menu.Append(d_wireframe, 'Wireframe',
                                'Switch to wireframe mode.')
        self.Bind(wx.EVT_MENU, self.OnDisplayModeWireframe, id=d_wireframe)
        d_shaded = wx.NewId()
        displaymode_menu.Append(d_shaded, 'Shaded', 'Switch to shaded mode.')
        self.Bind(wx.EVT_MENU, self.OnDisplayModeShaded, id=d_shaded)
        displaymode_menu.AppendSeparator()
        d_qhlr = wx.NewId()
        displaymode_menu.Append(d_qhlr, 'Quick HLR',
                                'Switch to Quick Hidden Line Removal mode.')
        self.Bind(wx.EVT_MENU, self.OnDisplayModeQHLR, id=d_qhlr)
        d_ehlr = wx.NewId()
        displaymode_menu.Append(d_ehlr, 'Exact HLR',
                                'Switch to Exact Hidden Line Removal mode.')
        self.Bind(wx.EVT_MENU, self.OnDisplayModeEHLR, id=d_ehlr)
        displaymode_menu.AppendSeparator()
        d_aon = wx.NewId()
        displaymode_menu.Append(d_aon, 'AntiAliasing On',
                                'Enable antialiasing.')
        self.Bind(wx.EVT_MENU, self.OnAntialiasingOn, id=d_aon)
        d_aoff = wx.NewId()
        displaymode_menu.Append(d_aoff, 'Antialiasing Off',
    ...
    def CreateRightToolbar(self):
        ...
        zoom_all = CreateMaskedBitmap(os.path.join(THISPATH, 'icons',
                                      'zoom_all.bmp'), 24, 24)
        pointer = CreateMaskedBitmap(os.path.join(THISPATH, 'icons',
                                                  'pointer.bmp'), 24, 24)
        zoom = CreateMaskedBitmap(os.path.join(THISPATH, 'icons',
                                               'zoom.bmp'), 24, 24)
        pan = CreateMaskedBitmap(os.path.join(THISPATH, 'icons',
                                              'pan.bmp'), 24, 24)
        rotate = CreateMaskedBitmap(os.path.join(THISPATH, 'icons',
                                                 'rotate.bmp'), 24, 24)
    ...
    def ExecPyFile(self, event):
        ...
        try:
            ...
            dlg = wx.MessageDialog(self, msg, "Method Error",
    ...
    def OnAbout(self, event):
        ...
        info.Copyright = "(C) 2008-2013 Andy Haywood"
        info.Description = "PythonOCC Interactive Console."
...
along with pythonOCC.  If not, see <http://www.gnu.org/licenses/>."""
    ...
    def _createbrowser(self):
        ...
        self.browser = b
        self.notebook.AddPage(b, "Help", True, 0)
    ...
    def _createpythonshell(self):
        ...
        self.notebook.AddPage(py, "Python shell", True, 0)
        self.pyshell = py
