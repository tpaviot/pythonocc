...
class GraphicsCanva3D(wx.Panel):
    ...
    def OnRightUp(self, event):
        ...
        if event.ControlDown() and event.ShiftDown():
            ...
            if ((abs(self.dragStartPos.x - pt.x) > 1) or
               (abs(self.dragStartPos.y - pt.y) > 1)):
                self._3dDisplay.Zoom_Window(self.dragStartPos.x,
                                            self.dragStartPos.y, pt.x, pt.y)
...
if __name__ == "__main__":
    ...
    class AppFrame(wx.Frame):
        def __init__(self, parent):
            ...
            shp = BRepPrimAPI_MakeTorus(400, 100).Shape
            self.canva._3dDisplay.DisplayShape(shp)
    app = wx.PySimpleApp()
