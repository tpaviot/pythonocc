...
try:
    ...
    raise ImportError('The Cheetah template is necessary.')
...
class AppFrame(wx.Frame):
    def __init__(self, parent, title, x, y):
        wx.Frame.__init__(self, parent=parent, id=-1, title=title,
                          pos=(x, y), style=wx.DEFAULT_FRAME_STYLE,
                          size = (400, 300))
...
class ShapeAnalyzer:
    ...
    def translateToPositiveSpace(self):
        ...
        has been translated into positive space the current shape and
        bounds are not modified:
        if you want to re-analyze the shape, create another analyzer
        from the new shape
        """
    ...
    def guessUnitOfMeasure(self):
        ...
        The idea is that usually, the dimenions are either in or mm,
        and its normally evident which one it is
        """
        dimList = [abs(self.xMax - self.xMin),
                   abs(self.yMax - self.yMin),
...
class Slicer:
    ...
    def execute(self):
        ...
        while zLevel < self.zMax:
            ...
            if not self.display is None and not slice is None:
...
class Slice:
    ...
    def addEdge(self, edge):
        ...
        #this could be simplified-- QuasiUnformDeflection probably handles
        # a line
        #correcly anyway?
...
class SVGExporter:
    """
    Writes a sliceset to specified file in SVG formatself.
    """
...
def printUsage():
    print """
        Usage: OccSliceLib <inputfile: STL or STEP> [sliceThickness]
            - inputfile [required] is an STL or STEP file, ending in .stl,
            .stp, or .step.
            - sliceThickness [optional] is in the same units as the object,
            and is optional
            defaults to 0.3mm or 0.012 in
            Creates an SVG output file compatible with skeinforge,
        in same directory as inputfile
    """
