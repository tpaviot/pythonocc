...
%include SGEOM_required_python_modules.i
...
%nodefaultctor Handle_GEOM_BaseDriver;
class Handle_GEOM_BaseDriver : public Handle_TFunction_Driver {
	public:
		%feature("autodoc", "1");
		Handle_GEOM_BaseDriver();
		%feature("autodoc", "1");
		Handle_GEOM_BaseDriver(const Handle_GEOM_BaseDriver &aHandle);
		%feature("autodoc", "1");
		Handle_GEOM_BaseDriver(const GEOM_BaseDriver *anItem);
		%feature("autodoc", "1");
		Handle_GEOM_BaseDriver & operator=(const Handle_GEOM_BaseDriver &aHandle);
		%feature("autodoc", "1");
		Handle_GEOM_BaseDriver & operator=(const GEOM_BaseDriver *anItem);
		%feature("autodoc", "1");
		static		Handle_GEOM_BaseDriver DownCast(const Handle_Standard_Transient &AnObject);

};
%extend Handle_GEOM_BaseDriver {
	GEOM_BaseDriver* GetObject() {
	return (GEOM_BaseDriver*)$self->Access();
	}
};
%feature("shadow") Handle_GEOM_BaseDriver::~Handle_GEOM_BaseDriver %{
def __del__(self):
	try:
		self.thisown = False
		GarbageCollector.garbage.collect_object(self)
...
%nodefaultctor GEOM_BaseDriver;
class GEOM_BaseDriver : public TFunction_Driver {
	public:
		%feature("autodoc", "1");
		virtual		bool GetCreationInformation(std::string & theOperationName, std::vector<GEOM_Param, std::allocator<GEOM_Param>params);
		%feature("autodoc", "1");
		GEOM_Param & AddParam(std::vector<GEOM_Param, std::allocator<GEOM_Param>params, const char *name);
		%feature("autodoc", "1");
		virtual		const Handle_Standard_Type & DynamicType() const;

};
%extend GEOM_BaseDriver {
	Handle_GEOM_BaseDriver GetHandle() {
	return *(Handle_GEOM_BaseDriver*) &$self;
	}
};
%extend GEOM_BaseDriver {
	Standard_Integer __hash__() {
...
%feature("shadow") GEOM_BaseDriver::~GEOM_BaseDriver %{
def __del__(self):
	try:
		self.thisown = False
		GarbageCollector.garbage.collect_object(self)
...
class GEOM_Function : public MMgt_TShared {
	public:
		...
		%feature("autodoc", "1");
		TDF_Label GetHistoryEntry(const Standard_Boolean create=true);
		%feature("autodoc", "1");
		TDF_Label GetArgumentHistoryEntry(const TDF_Label &theArgumentRefEntry, const Standard_Boolean create=true);
		%feature("autodoc", "1");
		TCollection_AsciiString GetSingleValueAsString(int );
		%feature("autodoc", "1");
		Handle_TColStd_HArray1OfExtendedString GetArrayValueAsString(int );
		%feature("autodoc", "1");
		TDF_Label GetNamingEntry(const Standard_Boolean create=true);
...
%nodefaultctor GEOM_Param;
class GEOM_Param {
	public:
		%feature("autodoc", "1");
		GEOM_Param();
		%feature("autodoc", "1");
		void Set(const char *nm);

};
%feature("shadow") GEOM_Param::~GEOM_Param %{
def __del__(self):
	try:
		self.thisown = False
		GarbageCollector.garbage.collect_object(self)
