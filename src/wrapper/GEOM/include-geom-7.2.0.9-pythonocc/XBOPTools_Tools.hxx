...
#ifndef _XBOPTools_Tools_HeaderFile
#define _XBOPTools_Tools_HeaderFile
...
//! at parameters <aP1,aP2> <br>
  Standard_EXPORT   static  void MakeSectEdge(const XIntTools_Curve& aIC,const TopoDS_Vertex& aV1,const Standard_Real aP1,const TopoDS_Vertex& aV2,const Standard_Real aP2,TopoDS_Edge& aNewEdge) ;
  
//! Update the tolerance value for vertex  <aV> <br>
//! taking into account the fact that <aV> lays on <br>
//! the curve <aIC> <br>
  Standard_EXPORT   static  void UpdateVertex(const XIntTools_Curve& aIC,const Standard_Real aT,const TopoDS_Vertex& aV) ;
...
//! edges <br>
  Standard_EXPORT   static  Standard_Boolean IsBlocksCoinside(const XBOPTools_PaveBlock& aPB1,const XBOPTools_PaveBlock& aPB2) ;
  
//! Returns TRUE if PaveBlock <aPB> lays on the face <aF>, i.e <br>
//! the <PB> is IN or ON in 2D of <aF> <br>
  Standard_EXPORT   static  Standard_Boolean IsBlockInOnFace(const XBOPTools_PaveBlock& aPB,const TopoDS_Face& aF,const Handle(XIntTools_Context)& aContext) ;
...
//! resolution and corresp. tolerances' values of <aE1>, <aE2> <br>
  Standard_EXPORT   static  void CorrectRange(const TopoDS_Edge& aE1,const TopoDS_Edge& aE2,const XIntTools_Range& aSR,XIntTools_Range& aNewSR) ;
  
//! Correct shrunk range <aSR> taking into account 3D-curve <br>
//! resolution and corresp. tolerances' values of <aE>, <aF> <br>
  Standard_EXPORT   static  void CorrectRange(const TopoDS_Edge& aE,const TopoDS_Face& aF,const XIntTools_Range& aSR,XIntTools_Range& aNewSR) ;
