...
#ifndef _XBOPTools_Tools3D_HeaderFile
#define _XBOPTools_Tools3D_HeaderFile
...
//! Remove seam edges from face <aF> <br>
  Standard_EXPORT   static  void RemoveSims(const TopoDS_Face& aF,const Handle(XIntTools_Context)& aContext) ;
  
//! Remove seam edges from all faces of shape <aS> <br>
  Standard_EXPORT   static  void RemoveSims(const TopoDS_Shape& aS,const Handle(XIntTools_Context)& aContext) ;
...
//! (using  projection) <br>
  Standard_EXPORT   static  Standard_Boolean IsSplitToReverse1(const TopoDS_Edge& aE1,const TopoDS_Edge& aE2,const Handle(XIntTools_Context)& aContext) ;
...
//! Internal usage <br>
  Standard_EXPORT   static  void GetPlanes(const TopoDS_Edge& aSp,const TopoDS_Edge& aE2,const TopTools_IndexedDataMapOfShapeListOfShape& aEFMap2,const TopoDS_Edge& aE1,const TopoDS_Face& aF1,TopAbs_State& aST1,const Handle(XIntTools_Context)& aContext) ;
...
//! the edge <aE>   at parameter <aT>  towards to the <br>
//! material of the face <aF>. The value of shifting in <br>
//!  2D is  dt2D=XBOPTools_Tools3D::MinStepIn2d() <br>
...
//! If the distance  (<aP2>, <aPx>) > TolF => returns <aP1> <br>
  Standard_EXPORT   static  void PointToCompare(const gp_Pnt& aP1,const gp_Pnt& aP2,const TopoDS_Face& aF,gp_Pnt& aPx,const Handle(XIntTools_Context)& aContext) ;
  
//! Compute 3D-state for the point on the split edge <aSp> <br>
//! (with base edge <aE1> and the face <aF1>) comparing with <br>
//! the face <aF2> <br>
  Standard_EXPORT   static  void GetPlane(const TopoDS_Edge& aSp,const TopoDS_Edge& aE1,const TopoDS_Face& aF1,const TopoDS_Face& aF2,TopAbs_State& aST,const Handle(XIntTools_Context)& aContext) ;
...
//! Used in touch case  <aF1>/<aF2> <br>
  Standard_EXPORT   static  TopAbs_State GetStatePartIN2D(const TopoDS_Edge& aSp,const TopoDS_Edge& aEF1,const TopoDS_Face& aF1,const TopoDS_Face& aF2,const Handle(XIntTools_Context)& aContext) ;
  
  Standard_EXPORT   static  Standard_Boolean CheckSameDomainFaceInside(const TopoDS_Face& theFace1,const TopoDS_Face& theFace2,const Handle(XIntTools_Context)& theContext) ;
  
  Standard_EXPORT   static  Standard_Boolean ComputeFaceState(const TopoDS_Face& theFace,const TopoDS_Solid& theRef,const Handle(XIntTools_Context)& theContext,TopAbs_State& theState) ;
  
  Standard_EXPORT   static  Standard_Boolean TreatedAsAnalytic(const Standard_Real aTx,const gp_Pnt& aPx,const TopoDS_Edge& aEx,const TopoDS_Face& aFx,const TopoDS_Edge& aE1,const TopoDS_Face& aF1,const Standard_Real aTolTangent,const Standard_Real aTolRadius,TopAbs_State& aState,const Handle(XIntTools_Context)& aContext) ;
  
  Standard_EXPORT   static  Standard_Boolean TreatedAsAnalytic(const TopoDS_Face& aFx,const TopoDS_Edge& aSpE1,const TopoDS_Face& aF1,const Standard_Real aTolTangent,const Standard_Real aTolRadius,TopAbs_State& aState,const Handle(XIntTools_Context)& aContext) ;
