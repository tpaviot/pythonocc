...
class ModularBuilder(object):
    """
    This class generates a set of .i files integrated in one OCC.i script.
    The result is a simple _OCC.pyd and OCC.py script that enable better
    handling of different OCC classes.
    """
    def __init__(self, module, INC_PATH=environment.OCC_INC):
        self.MODULES = module
        self.MODULE_NAME = module[0]
        # the path where are the headers to parse can be OCC_INC
        # or SALOME_GEOM_INC
    ...
    def BuildEnumList(self):
        ...
        """
        try:  # error with BRepBndLib on Linux
    ...
    def write_renames_file(self):
        ''' some methods needs to be renamed. For instance, the Vertex()
        method of the TopoDS class, which is a static method, will be accessed
        under python with the name TopoDS_Vertex(). It conflicts with the
        TopoDS_Vertex class and then one of the class/method is shadowed by
        SWIG. As a consequence, all the static methods are renamed with the
    ...
    def process_by_ref_argument(self, argument_list):
        ''' process arguments passed by ref. Try to remove the by ref
        and pass by value.
        This will be achieved only if the argument passed by ref has defined
        a copy constructor.
        '''
    ...
    def write_function_arguments(self, mem_fun):
        ...
        default_value = {}
        FUNCTION_MODIFIED = False  # is the function modified by any byref ?
        ...
        for i in range(len(arguments)):
            ...
            #
            # Check if the parameter is a typedef defined in another
            # OCC package
            #
            argument_type_name = argument_types[0]
            #
            # The argument type_name may be Standard_Real, gp_Pnt etc.
            # The associated header must be added to the swig .if file in
            # order to
            # properly compile. This is then the strategy:
            # Example: V3d package is parsed
            #    - if the argument_type_name startswith V3d_, it's defined in
            # this module. Pass.
            #    - if the argument_type_name does'nt start with V3d,
            # (for ex gp_), then add the header
    ...
    def process_by_ref_return_type(self, return_type_string):
        ...
        const gp_Pnt
        so that the argument is returned by value. As a consequence, the
        ...
        # WARNING : the classes defined in such modules must have public
        # copy ctors
    ...
    def write_function(self, mem_fun, parent_is_abstract):
        ...
        if function_name in self.MemberfunctionDocStrings:
            key = function_name
        else:
            key = None
        # Detect virtuality
    ...
    def process_class(self, class_declaration):
        ...
        if not class_name in self.DERIVED:
        ...
        # Aim of the following lines is to handle the Hypothesis_Status
        # enum defined in the SMESH_Hypothesis class and add the corresponding
        # code to the SWIG file.

        # protected_enums is a list of encountered protected defined enums.
        # TODO: find a way to find automatically whether or not the enum
        # is protected
        protected_enums = ['PointLocation', 'Comparison', 'Logical',
                           'HypothesisType', 'VValue', 'IValueIndex',
                           'VValueIndex', 'SValueIndex', 'ValueIndex',
        ...
        for mem_fun in class_declaration.public_members:  # member_functions():
            ...
            if (function_name.startswith('~')):  # ignore destructor
                pass
            elif not class_name in self.MEMBER_FUNCTIONS_TO_EXCLUDE:
        ...
        if (class_name == 'TopoDS_Shape'):
            ...
            self.fp.write('\t\tfrom BRepTools import BRepTools_ShapeSet\n')
            self.fp.write('\t\ttopods_str, indx = state\n')
            self.fp.write('\t\tss = BRepTools_ShapeSet()\n')
            self.fp.write('\t\tss.ReadFromString(topods_str)\n')
            self.fp.write('\t\tthe_shape = ss.Shape(ss.NbShapes())\n')
            self.fp.write('\t\tlocation = ss.Locations().Location(indx)\n')
            self.fp.write('\t\tthe_shape.Location(location)\n')
    ...
    def CheckParameterIsTypedef(self, param_name):
        ...
        If this is a typedef that is not defined in that module, returns
        the module name.
        """
    ...
    def AddFunctionTransformation(self, t):
        """ This methods writes in a file all classes/methods that are
    ...
    def FindMemberFunctionsToExclude(self):
        """
        Build a dict about members functions to exclude (linkage
        issue for example)
        """
    ...
    def OSFilterHeaders(self, HXX_FILES):
        ...
        if sys.platform != 'win32':
            ...
            HXX_TO_EXCLUDE.append('Image_PixelFieldOfDColorImage.hxx')
            HXX_TO_EXCLUDE.append('BRepExtrema_DistanceSS.hxx')
    ...
    def WriteModuleEnums(self):
        ...
        try:  # BRepBndLib raises a pygccxml exception.
            for enum in self._mb.enumerations():
                if enum.name.startswith('%s_' % self.MODULE_NAME):
                    enum_name = enum.name
    ...
    def BuildModule(self):
        ...
        self._mb = module_builder.module_builder_t(files=[self._wrapper_filename],
