...
from templates import ClassTemplate, FunctionTemplate, GUIDTemplate, \
    ...
    ModuleTemplate, LicenseTemplate, PropertyTemplate
...
include_matcher = lambda c: not c.ignore
...
class module_matcher(matcher_base_t):
    ...
    def __call__(self, decl):
        return re.match("^(Handle_)*%s_"%self.module_name, decl.name)
...
class ModularBuilder(object):
    ...
    def write_wrapper(self):
        ...
        filename = os.path.join(self.build_path,'%s_Wrapper.hxx' % t)
    ...
    def build_modules(self):
        ...
        for module_name in self.module_names + self.cached_modules:
            try:
                ...
                continue
            for c in mb.decls(module_matcher(module_name), recursive=False):
                ns.adopt_declaration(c)
                mb.global_ns.remove_declaration(c)
            
            mb.global_ns.adopt_declaration(ns)

        for rem in mb.decls(lambda d: not isinstance(d, namespace_t), recursive=False):
            mb.global_ns.remove_declaration(rem)
    ...
    def configure_modules(self):
        ...
        # IGNORES #
        ###########
        for module in mb.namespaces():
            module.decls().include()
            module_conf = configuration.modules[module.name]
            module.classes(lambda c: c.name in module_conf.get('exclude', [])).exclude()
            for cls, excludes in module_conf.get('exclude_members', {}).items():
                module.classes(cls).mem_funs(lambda f: f in excludes).exclude()
        ...
        mb.classes(lambda c: bool(c.mem_funs("HashCode"))).add_template(HashTemplate())
    ...
    def write_modules(self):
        ...
        for module_name in self.module_names:
            ...
            module_file.write(module.main_template.render(module))
            module_file.close()
...
b = ModularBuilder(['Geom', 'gp'], rebuild_db=True)

#b = ModularBuilder(['GeomFill', 'gp', 'IntCurve', 'PNaming', 'StepSelect', 'MoniTool', 
#                    'TColGeom', 'GeomToStep','SelectBasics', 'GccInt', 'SelectMgr', 
#                    'GeomLProp', 'CPnts', 'IntImp', 'Standard', 'GeomAdaptor', 'TopTrans', 'MMgt', 'ApproxInt', 'Bnd', 'PGeom2d', 'Approx', 'Extrema', 'MAT', 'DBC'],rebuild_db=True)
mb = b._mb
#import pickle
#f = file("module_cache.pkl", 'w')
#pickle.dump(mb.global_ns, f)
#f.close()
