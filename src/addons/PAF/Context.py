...
def _operation_decorator(context, function, _operations):
    def __operation_decorator(*args, **kwargs):
        ...
        # getting rid of self as the first argument, a Context.*_operations
        ...
        # if 'name' in kwargs, set the name of the resulting Geom_object
        # to 'name'
        if 'name' in kwargs:
...
class ParametricModelingContext(object):
    ...
    def __init__(self, parameters, undo=False, commit=False, register_all_operations=True):
        ...
        a ParametricModelingContext always takes a Parameters instance
        as argument

        >>> from OCC.PAF.Parametric import Parameters
        >>> p = Parameters()

        Initialize context, such that every operation is added to the
        Undo/Redo stack

        Note that if in your operation, you can set an additional "undo
        argument
        If this you use this argument, than that operation will be added to
        the Undo / Redo stack
    ...
    def _register_all_operations(self):
        ...
        self.register_operations(self.basic_operations)
        self.register_operations(self.block_operations)
        self.register_operations(self.boolean_operations)
        self.register_operations(self.curve_operations)
        self.register_operations(self.group_operations)
        self.register_operations(self.healing_operations)
        self.register_operations(self.insert_operations)
        self.register_operations(self.local_operations)
        self.register_operations(self.measure_operations)
        self.register_operations(self.shapes_operations)
    ...
    def _initialize_operation(self):
        ...
        we wont be able to access these objects when they are overwritten by
        the newly generated classes ( wrapping with operation )
        '''
        # these are the methods that all Operations modules share
        no_ops = ('GetEngine', 'SetErrorCode', 'StartOperation', 'GetErrorCode'
                  'GetSolver', 'SetNotDone', 'AbortOperation',
                  'FinishOperation', 'IsDone', 'GetDocID', 'this', 'thisown')
        #======================================================================
        ...
        for i in dir(self):
            if i.endswith('operations') and not i.startswith('register'):
                ...
                # these classes will replace the
                # self.myEngine.GetIShapesOperations(self.docId) instance
                ...
                for j in dir(getattr(self, i)):
                    ...
                    if j.startswith('_'):
                        ...
                        func = _operation_decorator(self,
                ...
                # hash the old operation to the new one, such that
                # news operation instances can be registered
    ...
    def set_display(self, Viewer3d_instance):
        ...
        # If geom were already created before the display is initialized,
    ...
    def set_automatic_update(self, boolean_value):
        ''' Sets the AUTOMATIC_DISPLAY variable. Default is True. If set to
        False, the .solve() and
    ...
    def register_operations(self, *_operations):
        for _operation in _operations:
            try:
                ...
                raise AssertionError('[PAF] no solver found for class:', _operation.__class__)
    ...
    def set_parameter(self, name, value, does_commit):
        # call registered callbacks. Rules/Relations have to be updated
        # before the geometry is modified.
        # Be careful: just rules and Callbacks have to be called before the
        ...
        self.myEngine.SetInterpreterConstant(self.docId,
                                             TCollection_AsciiString(name),
                                             value,
                                             TCollection_AsciiString(""), True)
        #update registered solvers
    ...
    def get_shape(self, registered_object):
        ''' Returns the TopoDS_Shape for the registered object.
        This method enables interoperability
        with the OCC kernel. For instance for topology traversing,
        STEP export etc.
        '''
    ...
    def _create_presentation(self, obj, color=0):
        ...
        prs = TPrsStd_AISPresentation_Set(result_label,
                                          TNaming_NamedShape_GetID()).GetObject()
        prs.SetColor(color)
        prs.Display(True)
        self.pres[obj] = prs
        return True
    ...
    def get_presentation(self, geom_obj):
        ...
        # TPrsStd_AISPresentation().Set can create or retrieve the
        # AIS_Presentation
        # so, this is not re-creating the TPrsStd_AISPresentation
        # presentation
        prs = TPrsStd_AISPresentation_Set(result_label,
                                          TNaming_NamedShape_GetID()).GetObject()        
        return prs
