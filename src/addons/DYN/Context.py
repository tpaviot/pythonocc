...
class DynamicSimulationContext(ode.World):
    ...
    def register_post_step_callback(self, function):
        ''' Append the function to a list. These functions are called after
         each simulation step
        is completed. It can be useful, for instance, to refersh GUI, or send
        results to a queue etc.
        '''
    ...
    def set_display(self, display, yield_function=None):
        ...
        # Register a callback so that it's possible to move the view
        # with the mouse
    ...
    def set_animation_frame_rate(self, frame_rate):
        ''' Define the frame rate, i.e. the number of frames displayed in
        ...
        For instance, if the time_step is 0.1s, the max frame rate is 10
        (display updated.
        '''
    ...
    def register_shape(self, topods_shape, enable_collision_detection=False, use_boundingbox=False, use_sphere = False, use_trimesh=False):
        ...
        @param enable_collision_detection : tells whether the shape collisions
        have to be computed
        @param use_bounding_box : the geometry for collision detection is the
        bounding box of the shape
        @param use_trimesh : the geometry for collision detection is a 3D
        triangle mesh.
    ...
    def start_open_loop(self, quick=False):
        ...
        # Create the shape transformer outside the loop to have an
        # faster algorithm
        shape_trsf = gp_Trsf()
