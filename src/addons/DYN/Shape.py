...
class DynamicShape(ode.Body):
    ...
    def __init__(self, dyn_context, infinite_rotation=False):
        ''' function prototype:
        DynamicShape(self, parent_context, topods_shape,
        enable_collision_detection,use_boundingbox,use_trimesh )
        '''
    ...
    def use_sphere(self):
        ...
        The sphere radius is computed as the mean of the bounding box
        dimensions. As a consequence,
        if the TopoDS_Shape is a sphere, then the radius of the ode.GeomSphere
        will be the radius
        of the sphere
        '''
        xmin, ymin, zmin, xmax, ymax, zmax = get_boundingbox(self._shape, EPSILON)
    ...
    def _compute_trimesh(self, quality_factor=1.0):
        ''' Connect a ode.Trimesh to this body. The mesh is generated from the
        MSH subpackage.
        @TODO use the tesselation modules vertices lits
        and faces indices are passed to the trimesh.
        The default mesh precision is divided by the quality factor:
        - if quality_factor>1, the mesh will be more precise, i.e. more
        triangles (more precision, but also
        more memory consumption and time for the mesher,
        - if quality_factor<1, the mesh will be less precise.
        By default, this argument is set to 1 : the default precision of the
        mesher is used.
        '''
        ...
        # Before we set the vertices list to the trimesh, we must translate
        # them:
    ...
    def set_shape(self, topods_shape):
        ...
        if not self._inf_rotation:
            #===============================================================
            #    this is a common source of error, so defaulting to the finite
            #   rotation
            #    that avoids many cases of the common "bNormalizationResult"
            #  error
